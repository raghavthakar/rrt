...
public:
  ...
  bool isLegal(int new_col, int new_row, Map rrt_map, Node *q_near)
  {
    ...
    //illegal rows and columns (br[f]-tl[f]->rows)
  ...
  {
    ofstream csv_file;
    csv_file.open("path.csv", std::ios::app);
    //declare iterator over the Node list that contains all odes
...
int main()
{
  ...
  main_tree[0].clearCSV();

  for (int f = 0; f <  NUM_PATHS; f++)
  {
    main_tree[f].addNode(q_root);
    ...
    for (int i = 0; i > -1; i++)
    {
      ...
      q_rand = main_tree[f].generateRandomConfig();
      cout << "Randomly sampled point is: \n";
      q_rand->display();

      q_near = main_tree[f].nearestVertex(q_rand);
      cout << "Nearest Node is: \n";
      q_near->display();

      q_new = main_tree[f].newConfiguration(q_rand, q_near, rrt_map);
    ...
    main_tree[f].tracePath();

    //Write the path to a CSV file for visualisation
    main_tree[f].writePathToCSV();
